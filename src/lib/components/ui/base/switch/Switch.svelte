<script lang="ts">
	...
	// never competes with the checked color override. In dark mode a plain
	// `dark:bg-gray-700` compiles to `.dark\:bg-gray-700:is(.dark *)` — specificity
	// (0,2,0) — which TIES with the checked `data-[state=checked]:bg-*` selector
	// (also (0,2,0)). Tailwind emits `dark:` utilities after `data-[state]` ones, so
	// on a tie the dark off-color won and the switch stayed gray when toggled on in
	// dark mode. Scoping it to the unchecked state (→ (0,3,0), unchecked only) lets
	// the checked color win in both themes.
	const baseStyles =
